...
```{r}
PreQ_filtered$ID[PreQ_filtered$U_ID=="B160"] <- "4283F"
...
```{r}
PreQ_filtered$ID[PreQ_filtered$U_ID=="B55"] <- "4293F"
```
...
```{r}
PreQ_filtered$ID[PreQ_filtered$U_ID=="B45"] <- "4294F"
```

#4312
According to the symbols, the female ID will be changed to from 4312 to 4321.
```{r}
PreQ_filtered$ID[PreQ_filtered$U_ID=="B73"] <- "4321"
```
...
```{r}
PreQ_filtered$ID[PreQ_filtered$U_ID=="B299"] <- "4326F"
```
...
```{r}
PreQ_filtered <- PreQ_filtered[-115, ]
```
...
```{r}
PreQ_filtered$Color[PreQ_filtered$U_ID=="B269"] <- "Green"
PreQ_filtered$ID[PreQ_filtered$U_ID=="B269"] <- "4358F"
```
...
```{r}
PreQ_filtered$ID[PreQ_filtered$U_ID=="B153"] <- "4397"
```

#4397 
the Kangaroo entered ID no. 4397 instead of 4307. It will be changed to 4307.

```{r}
PreQ_filtered$ID[PreQ_filtered$U_ID=="B86"] <- "4307"
```

#4426

One student did not place a symbol and said "Do not know"

```{r}
PreQ_filtered <- filter(PreQ_filtered,Picture != "I Dont know")

```
#Error: The code did not work out well and the line was not deleted.

#4571

This is problematic, since two females used the same ID number and picture in the same session. Needs to be deleted later.

Both females in same class used the same ID, will be changed according to IP address to 45710 and 45719
...
Saving the duplicated IDs


```{r}
anyDuplicated(PreQ$ID)
```

Row #492 contains a teacher's response, needs to be deleted.

```{r}
PreQ_filtered <- filter(PreQ_filtered,ID != "Hassan Shafik")
```


#Continue of the cleaning progress
...
```{r}
#4657: the values are not duplicated in the Post, so I will add 1 to the first one.
```


```{r}
PreQ$ID[PreQ$U_ID=="A323"] <- "46571"
```

```{r}
PreQ$ID[PreQ$U_ID=="B401"] <- "4771F"
```
...
#Saving the cleaned file into csv

```{r}
write.csv(PreQ,"PreQ_filtered6_21.csv")
```
