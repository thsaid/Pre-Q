...
from zero, if Bartlett’s test is significant then it is good news (Field, 2009)
...
```{r}
PALSfit2 <- cfa(latents.model2, data = PALS)#stardard fit
...
Saving PALSf scores in a new file

```{r}
PALSfscores <- dplyr::select(.data = PALS, 
                          ID, Gender, Certificate)
```
COnvertng PALSfscores matrix to data frame
...
```
#Next step matching IDs with fscores
Now I need to merge the data from PALSfscores to the ID, gender, and certificate. WIll need to double check this. How to match each ID with the fscore. 
